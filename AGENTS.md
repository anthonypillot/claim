...
Claim is a small, read-only JSON HTTP API that aggregates free game giveaways across storefronts (Epic Games today; Prime Gaming, Steam, GOG, GamerPower planned). Bun workspace monorepo (`packages/**`) with one package: `packages/api`, built on **Bun + Elysia + TypeBox + pino**. See `docs/roadmap.md` (feature direction) and `docs/architecture.md` (target file tree).
