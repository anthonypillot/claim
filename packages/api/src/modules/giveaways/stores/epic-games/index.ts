...
import { fetchFreeGamesPromotions } from "./api.ts";
import { toGiveaways } from "./mapper.ts";

export async function fetchFreeGames(options: { locale: string; country: string }): Promise<Giveaway[]> {
  return toGiveaways(await fetchFreeGamesPromotions(options), options.locale);
}
