...
Claim is a small, read-only JSON HTTP API for currently free game giveaways. It aggregates
...
The schema lives in `packages/api/src/db/schema.ts`; generated SQL migrations live in
...
Use `bun run build:production` for a production bundle, and `bun run start` to run the built API
